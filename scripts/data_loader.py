...
# Streamlit is optional here: use secrets if available, else fallback to defaults/env
try:
    import streamlit as st  # type: ignore
    _GOLD_FROM_SECRETS = st.secrets.get("paths", {}).get("gold_dir")  # type: ignore[attr-defined]
except Exception:
    _GOLD_FROM_SECRETS = None

# ---- Глобальные пути и список известных GOLD-файлов ----
# Файл находится в my_ozon_analytics/scripts/data_loader.py
# GOLD_DIR располагается на уровень выше папки scripts: my_ozon_analytics/gold
GOLD_DIR: Path = Path(
    os.environ.get("GOLD_DIR")
    or _GOLD_FROM_SECRETS
    or (Path(__file__).resolve().parents[1] | Path("gold"))
)
...
KNOWN_FILES = {
    ...
    "mart_unit_econ.csv",
    "data_dictionary.csv",
    "forecast_sku_monthly.csv",
    "production_justification.csv",
    "assumptions.csv",
    "backtest.csv",
}
...
def coerce_date(df: pd.DataFrame, col="date"):
...
def _basic_hygiene(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Numeric hygiene for common metric columns, if present
    numeric_candidates = [
        "order_value_rub_sum", "promo_rub", "returns_rub",
...
REQUIRED = {
    "fact_sku_daily.csv": ["date","sku","shipped_qty","returns_qty","order_value_rub_sum","promo_rub","shipments","returns_rub"],
    "fact_sku_monthly.csv": ["period","sku","shipped_qty","order_value_rub_sum"],
    "mart_unit_econ.csv": ["sku","total_rev","net_revenue","margin","total_qty","returns_pct",
...
def _ensure_columns(df: pd.DataFrame, required: list[str]) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame({c: pd.Series(dtype="float64") for c in required})
    out = df.copy()
    for c in required:
        if c not in out.columns:
            out[c] = 0 if c not in ("date","period","sku") else ""
    return out

# опционально: инвалидируем кэш, если файлы в GOLD менялись
def _gold_mtime() -> float:
    try:
        return max(p.stat().st_mtime for p in GOLD_DIR.glob("*.csv"))
    except ValueError:
        return 0.0
...
def load_all() -> Dict[str, pd.DataFrame]:
    ...
    """
    _ = _gold_mtime()  # доступ к функции — триггер для кэша при изменениях
    GOLD_DIR.mkdir(parents=True, exist_ok=True)  # на всякий случай

    daily = _ensure_columns(load_table("fact_sku_daily.csv"), REQUIRED["fact_sku_daily.csv"])
    monthly = _ensure_columns(load_table("fact_sku_monthly.csv"), REQUIRED["fact_sku_monthly.csv"])
    mart = _ensure_columns(load_table("mart_unit_econ.csv"), REQUIRED["mart_unit_econ.csv"])
